...
<body>
  ...
  <div class="select-option1">
    <span class="order1">ORDER BY AGE</span>
    <?php
    $id1 = $_GET['id1'];
    $name = $_GET['id2'];
    $id = $_GET['id3'];
    ...
    if ($id1 == 'ASC') {
      echo "
                        ...
                        <select name='orderr' class='select' onchange='location=this.value;'>
                        <option value='list1.php?do_id=" . $id . "&do_name=" . $name . "' > none ..</option>
                        
                        <option value='list_order.php?id1=ASC&id2=" . $name . "&id3=" . $id . "' selected >ASC</option>
      ...
      echo "
                                ...
                                <option value='list_order.php?id1=DESC&id2=" . $name . "&id3=" . $id . "' selected>DESC</option>
  ...
  <div class="main" id="tables">
    ...
    <table id="table2" cellspacing="0">
      ...
      if (isset($_GET['id1'])) {
        ...
        $sql1 = " SELECT * ,year(CURDATE())- year(dob) as age FROM `hospital`.`info` WHERE  `info`.`appoint`='$id2' ORDER BY `age` $id1 ; ";
        $data = $con->query($sql1);
...
for(let i=((index1*11)-10) ; i<=(((index1*11)-10)+10) ; i++)
  ...
  update_d=document.getElementsByClassName('row')[i];
   ...
   for(i=0 ; i <document.getElementsByClassName('row11').length ; i++)
   {

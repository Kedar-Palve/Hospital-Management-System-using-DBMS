...
$sql1 = " SELECT count(`p_name`) as count1 FROM `hospital`.`old_info`; ";
...
if (isset($_GET['id'])) {
  // $sql=" DELETE FROM `hospital`.`old_info`;";
...
<body>
  ...
  <div class="select-option1 select-option1d">
    <!-- <input type="number" class="" placeholder="Delete records . " max=""> -->
    <h3> Delete All Records</h3>

    <a href="patient.php?id='delete'">
  ...
  <div class="main" id="tables">
    ...
    <table id="table2" cellspacing="0">
      ...
      // $sql=" SELECT * FROM `hospital`.`info` ";
      $sql = " CREATE or REPLACE VIEW `hospital`.virtual_v as SELECT * ,
               year(CURDATE())- year(dob) as age FROM `hospital`.`old_info`;";

      //$sql=" SELECT * ,year(CURDATE())- year(dob) as age FROM `hospital`.`info`; ";
      $con->query($sql);
      $dd = "SELECT * from `hospital`.virtual_v;";
      $data = $con->query($dd);
   ...
   <tr class='row dd'>
    ...
    <td class='name1'>
    <p  name='p_name' class='input_tag ddd' required  >" . $result['textarea'] . "</p>
  ...
  <script>
    ...
    let disp = document.getElementById('select').selectedIndex;
    console.log(disp);
    if (disp > -1) {
